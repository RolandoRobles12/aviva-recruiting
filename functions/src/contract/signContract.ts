...
async function resolveOnboardingStageId(
  ...
  try {
    ...
    const isOnboardingName = (n: string) =>
      n === 'onboarding' || n === 'inducción' || n === 'induccion';
    // Exact match first — "Onboarding" must not match "Onboarding Iniciado".
    const exact = stages.find((s) => isOnboardingName(s.name.toLowerCase()));
...
export const signContract = onRequest(
  ...
  async (req, res) => {
    ...
    if (apiKey && viterbitCandidateId) {
      try {
        await patchViterbitCandidateFile(viterbitCandidateId, 'contrato_firmado', pdfUrl, apiKey);
      } catch (err) {
