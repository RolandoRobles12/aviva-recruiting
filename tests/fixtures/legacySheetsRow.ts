// The onboarding row exactly as sheetsService.ts built it before the Sheets
// integration became configurable (copied from git, not edited), used as the
// reference in tests/sheetsRow.test.ts. Do not "fix" anything here: its only
// job is to remember what the old code wrote.
...
export function legacyRow(
  ...
  const row: string[] = [
    ...
    /* 50 Fecha ejecución */                     fmtDateTime(now),
